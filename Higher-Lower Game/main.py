...
def comparison(answer, right_answer):
...
def right_answer(option_A, option_B):
...
score = 0
rightanswer = right_answer(option1, option2)
end_game = False
while not end_game:
    if option1 == option2:
        option2 = random.choice(data)
    print(f"Option A: {format_data(option1)}")
    print(vs)
    print(f"Option B: {format_data(option2)}")
    answer = input("Who has more followers? A or B:\t").lower()
    if answer == 'a':
        answer = option1
    elif answer == 'b':
        answer = option2
    if comparison(answer, rightanswer):
        score += 1
        clear()
